...
from keyboards.default.menyu import Ovqatlar, zakaz_ber_ovqatlar, shashlik, zakaz_ber_shashlik, Fanta, Pepsi, piva, \
    Cola, Menyu, Ichimliklar
...
async def tovuq(message:types.Message):
    await message.answer_photo(photo="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRFsnQd54mHYQ3KhLs27OZ8XivUwplaeA_irA&usqp=CAU",
                               caption="<b>Bu Tovuq \nNarxi: bepul</b>",
                               reply_markup=zakaz_ber_ovqatlar())
...
@dp.message_handler(Text(equals="🛒Savatga qushish"))
...
@dp.message_handler(Text(equals="🗑 Savatga qushish"))
async def zakaz_pepsi(message: types.Message):
...
@dp.message_handler(Text(equals="🛒 Savatga qushish"))
async def zakaz_cola(message: types.Message):
...
@dp.message_handler(Text(equals="🛒 Savatga-qushish"))
async def zakaz_fanta(message: types.Message):
...
async def tovuq_shashlik (message:types.Message):
    await message.answer_photo(photo="https://bazarplus.uz/wp-content/uploads/2023/01/tovuq-kichik.jpg",
                               ...
                               reply_markup=zakaz_ber_shashlik())
